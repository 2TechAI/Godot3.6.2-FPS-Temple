...
# Menu open flag - prevents shooting when in-game menu is visible
var menu_open = false;
...
func _ready() -> void:
	...
	arsenal["mk_23"] = weapon.weapon.new(self, "mk_23", 2.0, 12, 160, 12, 40, 1.2);
	
	# Create glock 17 using weapon class
	arsenal["glock_17"] = weapon.weapon.new(self, "glock_17", 3.0, 12, 160, 12, 35, 1.2);
	
	# Create kriss using weapon class
	arsenal["kriss"] = weapon.weapon.new(self, "kriss", 6.0, 32, 160, 33, 25, 1.5);
...
func _weapon(_delta) -> void:
	...
	input["inspect"] = int(Input.is_action_just_pressed("KEY_F"));
	...
	if not character.input["sprint"] or not character.direction:
		if input["inspect"]:
			arsenal.values()[current]._inspect(_delta);
...
func _unhandled_input(event):
	if event is InputEventKey:
		if event.pressed:
			var anim = arsenal.values()[current].get_anim()
